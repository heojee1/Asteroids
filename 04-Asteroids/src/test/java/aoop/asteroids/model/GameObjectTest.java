...
import aoop.asteroids.model.game_object.GameObject;
...
class GameObjectTest {
	...
	void testNextStep() {
		...
		for (int i = 0; i < 1000; i++) {
			...
			assertEquals(
					...
					(GameFrame.WINDOW_SIZE.width + locationBeforeStep.getX() + obj.getVelocity().getX()) % GameFrame.WINDOW_SIZE.width,
					0.000001
			);
			assertEquals(
					locationAfterStep.getY(),
					(GameFrame.WINDOW_SIZE.height + locationBeforeStep.getY() + obj.getVelocity().getY()) % GameFrame.WINDOW_SIZE.height,
					0.000001
			);
		}
	}
	...
	private GameObject getRandomGameObjectImplementation() {
		...
		Point.Double location = new Point.Double(rng.nextDouble(GameFrame.WINDOW_SIZE.width), rng.nextDouble(GameFrame.WINDOW_SIZE.height));
