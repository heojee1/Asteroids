...
class GameTest {
	...
	void testInitializeGameData() {
		Game game = new SingleplayerGame(""); // The constructor calls initializeGameData().
		...
		assertFalse(game.getMyShip().isDestroyed());
		assertEquals(0, game.getMyShip().getScore());
		...
		game.getMyShip().destroy();
		for (int i = 0; i < 10; i++) {
			game.getMyShip().increaseScore();
		}
